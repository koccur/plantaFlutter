...
class _AddPlantRoute extends State<AddPlantRoute> {
    ...
    return Container(
      child: Column(children: <Widget>[
        Row(children: <Widget>[Checkbox(value: checkboxWaterValue, onChanged: (bool newValue) {
          setState(() => checkboxWaterValue = newValue);
        ...
        if (checkboxWaterValue)Column(children: <Widget>[
          Row(children: <Widget>[
            ...
            Expanded(child: Slider(value: _plant.water.intensity.toDouble(),
              onChanged: !checkboxWaterValue ? null : (double newIntensity) {
                setState(() => _plant.water.intensity = newIntensity.round());
              },
              max: 3.0,
              min: 1.0,
              divisions: 2,
          ...
          Row(children: <Widget>[
            ...
            Container(//                  todo: check why is stays with value from field
              child: TextFormField(validator: (value) => value.isEmpty ? "write text" : null,
                initialValue: _plant.water.frequency.toString(),
                keyboardType: TextInputType.number,
                onChanged: (String value) => _plant.water.frequency = int.parse(value),),
          ...
          Container(child: Row(children: <Widget>[
            ...
            Text('Last activity'),
            Container(child: Row(children: <Widget>[Container(child: Text(_getFormattedDate(_plant.water.lastActivity)),
              margin: EdgeInsets.only(right: 8),), IconButton(onPressed: () =>
                _selectDate(
        ...
        if (checkboxFertValue)Column(children: <Widget>[
          ...
          Row(children: <Widget>[
            Container(padding: EdgeInsets.only(left: 42),
              margin: EdgeInsets.only(bottom: 8, top: 8),
              child: Text('Frequency (quarter)'),),
            Container(child: TextFormField(validator: (value) => value.isEmpty ? "write text" : null,
                initialValue: _plant.fertilization.frequency.toString(),
                keyboardType: TextInputType.number,
                onChanged: (String value) => _plant.fertilization.frequency = int.parse(value)),
              margin: EdgeInsets.only(left: 16),
              height: 50,
              width: 50,)
          ],),
          Container(child: Row(children: <Widget>[
            ...
            Text('Last activity'),
            Container(child: Row(children: <Widget>[
              Container(child: Text(_getFormattedDate(_plant.fertilization.lastActivity)), margin: EdgeInsets.only(
                  right: 8),),
  ...
  Widget sunnyAndLocContainer(context) {
    return Container(child: Column(children: <Widget>[
      Row(children: <Widget>[
        ...
        Expanded(child: Slider(value: _plant.place.sunnyLevel.toDouble(),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(appBar: addPlantBarFunc(context),
        body: Container(child: Form(key: _formKey,
            child: ListView(children: <Widget>[
              ...
              TextFormField(validator: (value) => value.isEmpty ? "write text" : null,
                  ...
                  onChanged: (String value) => _plant.name = value),
              ...
              TextFormField(initialValue: _plant.notes,
                  ...
                  onChanged: (String value) => _plant.notes = value),
            ],)),));
  }
  ...
  _initializePlantData(context) {
    ...
    if (plantTemp == null) {
      _plant = new Plant(uid: null,
          water: new Water(lastActivity: _getFormattedDate(new DateTime.now().toString()), frequency: 1, intensity: 1),
          place: new Place(roomName: '', sunnyLevel: 1),
          picture: null,
          notes: '',
          fertilization: new Fertilization(
              lastActivity: _getFormattedDate(new DateTime.now().toString()), frequency: 1, intensity: 1),
          name: '');
    } else {
